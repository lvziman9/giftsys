...
def _select_activity(key: str) -> dict[str, Any] | None:
...
def _render_building_tab() -> None:
    ...
    with st.form("add_building_form"):
        building_name = st.text_input("新增楼宇")
    ...
    for row in rows:
        ...
        with cols[-1]:
            if st.button(
                ...
                try:
                    log_reschedule_contact(row["_claim_id"], admin_id=admin["id"])
                    st.success(f"已记录联系改期：{row['员工']}")
                except ValueError as exc:
                    st.error(str(exc))
