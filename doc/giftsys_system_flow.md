...
9. 活动超过结束日期后自动进入“历史活动”，不再出现在普通活动管理下拉栏。
...
10. 结束日期早于今天的活动自动进入历史活动，历史活动只能作为模板再次发布。
...
flowchart LR
    subgraph ADMIN["管理端"]
        ...
        A4["发布活动"]
        A41["活动管理<br/>编辑有效期内活动 / 下线 / 恢复 / 调库存"]
        A42["历史活动<br/>搜索 / 查看数据 / 再次发布"]
        A5["预约管理"]
    ...
    subgraph SYSTEM["系统自动动作"]
        S1["生成默认 timeslots"]
        S2["生成活动上线通知"]
        S21["同步已结束活动<br/>结束日期早于今天"]
        S22["历史活动模板复用<br/>创建全新活动"]
        S3["资格 / 楼宇 / 库存过滤"]
        S4["预约占用库存"]
        S5["生成领取凭证"]
        S6["取消或过期释放库存"]
